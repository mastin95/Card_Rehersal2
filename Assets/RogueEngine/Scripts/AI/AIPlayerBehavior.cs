...
namespace RogueEngine.AI
{
    ...
    public class AIPlayerBehavior : AIPlayer
    {
        ...
        public override void Update()
        {
            ...
            if (character != null && game_data.CanControlCharacter(player_id, character) && game_data.IsCharacterTurn(character) && !gameplay.IsResolving())
            {
        ...
        public void SelectCost()
        {
            ...
            if (game_data.selector != SelectorType.None)
            {
                if (character.mana >= 0)
                {
                    int choice = rand.Next(0, character.mana + 1);
