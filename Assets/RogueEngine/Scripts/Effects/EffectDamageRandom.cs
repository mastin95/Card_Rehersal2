...
namespace RogueEngine
{
    ...
    public class EffectDamageRandom : EffectData
    {
        ...
        {
            int damage = GetDamage(logic.WorldData, ability, caster, card);
            logic.DamageCharacter(caster, target, damage);
        }

        public int GetDamage(World world, AbilityData ability, BattleCharacter caster, Card card)
        {
            int seed = world.GetLocationSeed(384);
