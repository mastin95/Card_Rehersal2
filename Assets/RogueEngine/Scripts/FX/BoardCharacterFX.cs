...
namespace RogueEngine.FX
{
    ...
    public class BoardCharacterFX : MonoBehaviour
    {
        ...
        private void OnAbilityEffect(AbilityData iability, Card card, BattleCharacter target)
        {
            if (iability != null && card != null && target != null)
            {
                if (target.uid == bcharacter.GetUID())
                {
                    ...
                    FXTool.DoProjectileFX(iability.projectile_fx, GetFXSource(card), FXTransform, damage_estimate);
        ...
        {
            BoardCharacter character = BoardCharacter.Get(caster.owner_uid);
            BoardCharacterFX cfx = character?.GetComponent<BoardCharacterFX>();
            if (cfx != null)
                return cfx.FXTransform;
            return null;
        }
