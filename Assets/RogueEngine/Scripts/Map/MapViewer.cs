...
namespace RogueEngine.Client
{
    ...
    public class MapViewer : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            bool drag = MouseInput.IsLeftHold();
            Vector3 worldPosition = Camera.main.ScreenToWorldPoint(MouseInput.GetMousePosition());
            ...
            //Move keyboard
            if (KeyInput.IsKeyHold(Key.A) || KeyInput.IsKeyHold(Key.LeftArrow))
                move_vect = Vector3.left * scroll_speed_key * Time.deltaTime;
            if (KeyInput.IsKeyHold(Key.D) || KeyInput.IsKeyHold(Key.RightArrow))
                move_vect = Vector3.right * scroll_speed_key * Time.deltaTime;

            float scroll = MouseInput.GetMouseScroll();
            if (Mathf.Abs(scroll) > 0.1f)
                move_vect = Vector3.right * scroll_speed_wheel * scroll * Time.deltaTime;
