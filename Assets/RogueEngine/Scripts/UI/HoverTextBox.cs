...
namespace RogueEngine.UI
{
    ...
    public class HoverTextBox : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            {
                transform.position = BattleUI.MouseToWorld(MouseInput.GetMousePosition(), 16f);
                panel_left.SetVisible(transform.position.x > 0f);
                panel_right.SetVisible(transform.position.x <= 0f);

                bool bottom = MouseInput.GetMousePosition().y < 400f;
                if (bottom)
                {
            ...
            if (current == null && current_ui == null && IsVisible())
                Hide();
        }
        ...
        {
            return panel_left.IsVisible() || panel_right.IsVisible();
        }
